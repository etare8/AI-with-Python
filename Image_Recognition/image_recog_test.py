...
# READING ALL IMAGES:
directory_path = r".\fiber_analysis"
all_files_list = os.listdir(directory_path)

files_images = []
for name in all_files_list:
    if ('test' and '.jpeg') in name:
        files_images.append(name)
print(files_images)

#---------------------------------------------------

# FOR CYCLE:
# enumerate takes a list and gives back index and obect of the list for each position
for (index_im, path) in enumerate(files_images): 
    # Load the image to analize
    img_load = cv2.imread(r'.\fiber_analysis' + f"\{path}")

    # Convert to grayscale and apply thresholding
    img_grey = cv2.cvtColor(img_load, cv2.COLOR_RGB2GRAY)
    _, img_thresh = cv2.threshold(img_grey, 10, 255, cv2.THRESH_BINARY_INV)
    ...
    print(f"\nNumber of Fibers (Image #{index_im+1}):", ellips_count, "\n")
    ...
    cv2.imshow(f"Test Image {index_im+1}",img_resized)
    cv2.waitKey(0)
